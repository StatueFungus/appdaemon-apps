        ...
        for entity in sorted(entities):
            ...
            try:
                ...
                if "battery_level" in entity or "battery" in entity:
                    # battery_level = float(await self.get_state(entity))
                    battery_level = float(states[entity]["state"])

                # check entity attributes for battery levels
                if not battery_level:
                    for attr in LEVEL_ATTRIBUTES:
                        # battery_level = float(await self.get_state(entity, attribute=attr))
                        battery_level = float(states[entity]["attributes"].get(attr))
                        break
            ...
            if battery_level and battery_level <= check_config["min_level"]:
                ...
                self.lg(
                    ...
                    # f"{hl(f'battery → {hl(float(battery_level))}')}%",
                    f"{hl(f'battery → {hl(float(battery_level))}')}% | " f"last update: {last_updated}",
